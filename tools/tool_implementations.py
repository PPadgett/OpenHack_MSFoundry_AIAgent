            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"{func.__name__} failed; retrying in {backoff}s: {e}"
                        )
                        import time
...
def idempotent_post(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid idempotency key (must be UUID): {idempotency_key}"
            )
...
class MenuLookup:
    ...
    def lookup(
        ...
        topping_category: Optional[str] = None,
    ) -> List[MenuItem]:
        """
        Retrieve menu items.
...
class AllergenLookup:
    ...
    def lookup(item_id: str, allergen_filter: Optional[str] = None) -> AllergenData:
        ...
        allergen_data = AllergenData(
            ...
            preparation_notes=data.get(
                "preparation_notes", "Prepared in shared kitchen environment."
            ),
        )

        logger.info(
            f"allergen_lookup for {item_id}: {len(allergen_data.allergens)} allergens found"
        )
...
class PriceCalculator:
    ...
    def calculate(
        ...
        logger.info(
            f"price_calc: subtotal={calculation.subtotal}, tax={calculation.tax}, total={calculation.total}"
        )
...
class PizzaQuantityEstimator:
    ...
    @staticmethod
    def estimate(
        number_of_people: int, appetite_level: str, slices_per_pizza: int = 8
    ) -> PizzaQuantityRecommendation:
        """
        ...
        slices_per_person = PizzaQuantityEstimator.SLICES_BY_APPETITE[
            normalized_appetite
        ]
        safety_buffer = max(1, math.ceil(number_of_people * 0.1))
        estimated_total_slices = math.ceil(
            (number_of_people * slices_per_person) + safety_buffer
        )
        ...
        if recommended_pizzas >= 3:
            notes.append(
                "Suggestion: split across cheese, meat, and veggie options for variety."
            )
        else:
            notes.append(
                "Suggestion: include at least one broadly popular option like classic cheese."
            )
...
class OrderSubmitter:
    ...
    def submit(
        ...
        if allergen_flags:
            logger.warning(
                f"Order {order_id} submitted with allergen flags: {allergen_flags}"
            )

        logger.info(
            f"order_submit succeeded: order_id={order.order_id}, status={order.status}"
        )
...
class HumanHandoff:
    ...
    @staticmethod
    def escalate(
        reason: str, context: Optional[Dict[str, Any]] = None, priority: str = "normal"
    ) -> Dict[str, Any]:
        """
        ...
        logger.warning(
            f"Escalation created: ticket_id={data.get('ticket_id')}, reason={reason}, priority={priority}"
        )
